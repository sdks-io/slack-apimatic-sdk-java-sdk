...
```java
PinsApi pinsApi = client.getPinsApi();
```
...
pinsApi.pinsAddAsync(token, channel, null).thenAccept(result -> {
...
pinsApi.pinsListAsync(token, channel).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
...
pinsApi.pinsRemoveAsync(token, channel, null).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
